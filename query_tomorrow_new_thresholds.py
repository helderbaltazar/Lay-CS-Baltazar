...
targets = {
    "0-1": {"threshold": 94.00, "matches": []},
    "0-2": {"threshold": 94.00, "matches": []},
    "0-3": {"threshold": 99.20, "matches": []},
    "1-3": {"threshold": 99.31, "matches": []},
}
    ...
    if p.target_score in targets:
        if p.power_score is not None and p.power_score >= targets[p.target_score]["threshold"]:
            targets[p.target_score]["matches"].append(
                {"home": m.home_team, "away": m.away_team, "score": p.power_score}
            )
