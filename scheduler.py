...
def run_daily_scan():
    from run_real_injection import ensure_data_in_db
    import datetime
    print(f"[{datetime.datetime.now()}] Iniciando scan unificado (Apenas Hoje)...")
    try:
        ensure_data_in_db()
        update_layback_bots()
    except Exception as e:
        print(f"Erro no scan diário: {e}")
        send_message(f"❌ Erro crítico no scan diário: {e}")
